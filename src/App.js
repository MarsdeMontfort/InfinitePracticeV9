...
export default function App() {
  ...
  const [usedQuestionIds, setUsedQuestionIds] = useState(() => {
    const stored = localStorage.getItem("used_question_ids");
    return stored ? new Set(JSON.parse(stored)) : new Set();
  });
  // Stats per question (reset with Qbank or by itself)
  ...
  const [resetKey, setResetKey] = useState(0); // force rerender for state reset

  // The "available" pool, always updated from storage/state
  const pool = ALL_QUESTIONS.filter(
    (q) =>
      q.category.some((c) => selectedCats.has(c)) && !usedQuestionIds.has(q.id)
  );

  // Helper: category question counts for pool only
  const getCategoryCount = (category) =>
    pool.filter((q) => q.category.includes(category)).length;

  // Counts (Unused, Correct, etc.) — all only among pool questions
  ...
  // Clamp block size if pool shrinks
  useEffect(() => {
  ...
  // ---- When finishing a block, add questions to "used" ----
  const markBlockUsed = useCallback(() => {
    const updatedUsed = new Set(usedQuestionIds);
    testQuestions.forEach((q) => updatedUsed.add(q.id));
    setUsedQuestionIds(updatedUsed);
    localStorage.setItem("used_question_ids", JSON.stringify([...updatedUsed]));
  }, [testQuestions, usedQuestionIds]);

  // ---- Home page: Reset everything for fresh Qbank ----
  const resetQbank = () => {
    if (
      window.confirm(
        "Reset Qbank? This will let you see all questions again as 'unused'."
      )
    ) {
      ...
      setResetKey((k) => k + 1); // force re-render
    }
  };
  ...
  // Record answers and stats as before
  ...
  // When user leaves results page (or hits home), mark those Qs as "used"
  useEffect(() => {
    if (stage === "result") {
      markBlockUsed();
    }
